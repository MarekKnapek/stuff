...
# undef assert
	...
	void assert(bool const& expr);

	void unoverride_func(char* orig_func, unsigned n_bytes, char* backup_buff);
...
void (::mk::assert)(bool const& expr)
{
	if(!expr)
	{
...
void (::mk::unoverride_func)(char* orig_func, unsigned n_bytes, char* backup_buff)
{
	DWORD orig_perm = 0;
	BOOL const vp_ret1 = ::VirtualProtect(orig_func, n_bytes, PAGE_EXECUTE_READWRITE, &orig_perm);
	std::memcpy(orig_func, backup_buff, n_bytes);
	DWORD dummy = 0;

// ==UserScript==
// @id             iitc-plugin-portals-list@teo962
// @name           IITC plugin: show list of portals2
// @category       Info
// @version        1.2.1.20150917.154202
// @description    [jonatkins-2015-09-17-154202] Display a sortable list of all visible portals with full details about the team, resonators, links, etc. 2
...

{
	title: "Missing my r8?",
	value: function(portal){
		if(portal.options.data.level == 8){
			return "no";
		}
		...
		var found = false;
		var len = resonators.length;
		for(i = 0; i != len; ++i){
			if(resonators[i].level == 8 && resonators[i].owner == window.missmyrname){
				found = true;
				break;
			}
		}
...
{
	window.missmyrgetname = function(){
		var name = localStorage.getItem("missmyrname");
		if(!name || name == ""){
			name = window.PLAYER.nickname;
		}
		window.missmyrname = name;
		return name;
	};

	window.missmyrchange = function(e){
		var name = e.target.value;
		window.missmyrname = name;
		localStorage.setItem("missmyrname", name);
	};

	window.missmyrdialog = function(){
		var html = 'Agent name: <input type="text" value="' + window.missmyrname + '" onchange="window.missmyrchange(event);">';
		window.dialog({
			html: html,
			title: "Missing my r8?",
			modal: false
	...
	window.missmyrgetname();
	$('#toolbox').append('<a onclick="window.missmyrdialog();">Missing my r8?</a> ');
}
